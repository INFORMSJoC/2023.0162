...
'''
Implementation of a spatial branch and bound algorithm that uses interval Newton methods to determine if the current box contains a feasible point.
'''
...
def sBaB_newton(problem, tol=0.01, maxiter=10000, maxtime=10800, enlarge_box_constraints=False, extended_miranda=True, use_obbt=False, use_fbbt=(False, 10), var_lifting=False, debug=True, narrow_box=True, narrow_box_tol=1e-5, local_solver='ipopt', transform_interval=1, tunnel_approach=False, strict_removal=True):
    # enlarge starting box by 1 and model box constraints as inequality constraints
    if enlarge_box_constraints:
        prob = transform_problem(problem)
    else:
        prob = problem
    # perform the slack variable lifting_approach (optional)
    if var_lifting:
    ...
    while v_ub - v_lb > tol and k < maxiter and time()-starting_time < maxtime and L:
        ...
        current = L.pop(index)
        # branch the current box into two subboxes along the longest edge
        branch_list = branch(current[0])
        #if debug:
        #    print(''.join(['Length of List: ', str(len(branch_list))]))
        if debug:
            print(''.join(['Length of List: ', str(len(L))]))
        ...
        if extended_miranda:
            for i in branch_list:
                ...
                if cons_i is None:
                   if debug:
                    ...
                    # OPTIONAL: change this to interval arithematic
                    # lb_i = interval_arithmetic(prob.symobj, bnds_i, prob.vars)[0]
                    ...
                    if len(cons_i) > len(bnds_i):
                        ...
                        ub = zentrischeform(prob.symobj, bnds_i, prob.vars)[1]
                    else:
                        # determine upper bound for current box
                        ub = interval_newton_method(prob, bnds_i, cons_i, narrow_box_flag=narrow_box, narrow_epsilon=narrow_box_tol, local_solver=local_solver)
            ...
            for i in branch_list:
                ...
                L.append((bnds_i, lb_i))
                # determine upper bound for current box
                ub = interval_newton_method(prob, bnds_i, prob.cons, narrow_box_flag=narrow_box, narrow_epsilon=narrow_box_tol, local_solver=local_solver)
                if ub is None or ub < lb_i: # if no ub is determined or ub < lb (through rounding) - do not continue
                    pass
                else:
                    upper_bounds.append((bnds_i, ub))
